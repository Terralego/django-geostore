...
GEOSTORE_TILE_HOSTNAMES = getattr(settings, 'GEOSTORE_TILE_HOSTNAMES', [])

# Define a default maximum zoom value for tile generation. Can be overrided in layer settings.
GEOSTORE_MAX_TILE_ZOOM = getattr(settings, 'GEOSTORE_MAX_TILE_ZOOM', 15)

# Define a default minimum zoom value for tile generation. Can be overrided in layer settings.
GEOSTORE_MIN_TILE_ZOOM = getattr(settings, 'GEOSTORE_MIN_TILE_ZOOM', 10)

# For the moment, 4326 only
GEOSTORE_INTERNAL_GEOMETRY_SRID = getattr(settings, 'GEOSTORE_INTERNAL_GEOMETRY_SRID', 4326)
...
if getattr(settings, 'TERRA_TILES_HOSTNAMES', None) is not None:
    GEOSTORE_TILE_HOSTNAMES = getattr(settings, 'TERRA_TILES_HOSTNAMES')

if getattr(settings, 'MAX_TILE_ZOOM', None) is not None:
    GEOSTORE_MAX_TILE_ZOOM = getattr(settings, 'MAX_TILE_ZOOM')

if getattr(settings, 'MIN_TILE_ZOOM', None) is not None:
    GEOSTORE_MIN_TILE_ZOOM = getattr(settings, 'MIN_TILE_ZOOM')

if getattr(settings, 'INTERNAL_GEOMETRY_SRID', None) is not None:
    GEOSTORE_INTERNAL_GEOMETRY_SRID = getattr(settings, 'INTERNAL_GEOMETRY_SRID')

# Last workaround if HOSTNAME is set and not TERRA_TILES_HOSTNAMES:
if getattr(settings, 'HOSTNAME', None) and not GEOSTORE_TILE_HOSTNAMES:
    GEOSTORE_TILE_HOSTNAMES = [getattr(settings, 'HOSTNAME'), ]
