...
from django.contrib.gis.geos import GEOSException, GEOSGeometry, LineString, Point
...
class LayerViewSet(MultipleFieldLookupMixin, viewsets.ModelViewSet):
    ...
    @action(methods=["get", "post"], url_name="shapefile", detail=True)
            ...
            if request.user.has_perm("terra.can_export_layers"):
                ...
                if shape_file:
                    ...
                    response["Content-Disposition"] = (
                        "attachment; " f'filename="{layer.name}.zip"'
                    )
    ...
    def route(self, request, pk=None):
        ...
        response_data = {
            "request": {"callbackid": callbackid, "geom": geometry.json},
            "geom": route,
        }
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if "features" in request.data:
            try:
                ...
                return HttpResponseBadRequest(
                    "An error occured parsing " "GeoJSON, verify your data"
                )
        else:
