...
ACCEPTED_PROJECTIONS = ["urn:ogc:def:crs:OGC:1.3:CRS84", "EPSG:4326"]
...
class Layer(BaseUpdatableModel):
    ...
    def _initial_import_from_csv(self, chunks, options, operations):
        for chunk in chunks:
            entries = []
            for row in chunk:
                feature_args = {"geom": None, "properties": row, "layer": self}

                for operation in operations:
                    operation(feature_args, options)

                if not feature_args.get("geom"):
                    logger.warning("empty geometry," f" row skipped : {row}")
                    continue

                entries.append(Feature(**feature_args))
            Feature.objects.bulk_create(entries)

    def _complementary_import_from_csv(
        self, chunks, options, operations, pk_properties, fast=False
    ):
        for chunk in chunks:
            sp = None
            if fast:
            ...
            for row in chunk:
                self._import_row_from_csv(row, pk_properties, operations, options)
            if sp:
    ...
    def _import_row_from_csv(self, row, pk_properties, operations, options):
        ...
        if feature_args.get("geom"):
            Feature.objects.update_or_create(defaults=feature_args, **filter_kwargs)
        else:
            try:
                ...
                logger.warning(
                    "feature does not exist," " empty geometry," f" row skipped : {row}"
                )

    @topology_update
    @zoom_update
    def from_csv_dictreader(
        self,
        reader,
        pk_properties,
        options,
        ...
        if init:
            self._initial_import_from_csv(
                chunks=chunks, options=options, operations=operations
            )
        else:
    ...
    def from_geojson(self, geojson_data, id_field=None, update=False):
        ...
        if projection and not self.is_projection_allowed(projection):
            raise GEOSException(
                f"GeoJSON projection {projection} must be in " f"{ACCEPTED_PROJECTIONS}"
            )
    ...
    def to_shapefile(self):
        ...
        with TemporaryDirectory() as shape_folder:
            ...
            if self.geom_type is None:
                type_to_check = [
                    GeometryTypes.Point.name,
                    GeometryTypes.MultiPoint.name,
                    GeometryTypes.LineString.name,
                    GeometryTypes.MultiLineString.name,
                    GeometryTypes.Polygon.name,
                    GeometryTypes.MultiPolygon.name,
                ]
            else:
                type_to_check = self.get_geom_type_display()
            # Create one shapefile by kind of geometry
            for geom_type in type_to_check:
                schema = {"geometry": geom_type, "properties": self.layer_properties}
            ...
            for geom_type, shape in shapes.items():
                ...
                if not shape_size:
                    for filename in glob.iglob(
                        os.path.join(shape_folder, f"{geom_type}.*")
                    ):
                        os.remove(filename)
    ...
    def _fiona_shape_projection(self, shape):
        ...
        if projection and (
            len(projection) == 1
            or (len(projection) == 2 and projection.get("no_defs") is True)
        ):
    ...
    def update_geometries(self, features):
        ...
        for new_feature in features:
            ...
            nearest_feature = (
                self.features.filter(geom__bboverlaps=geometry)  # Bounding Box Overlap
                .annotate(hausdorff=ST_HausdorffDistance("geom", geometry.ewkb))
                .order_by("hausdorff")
                .first()
            )
    ...
    def layer_properties(self):
        ...
        return {prop: "str" for prop in results}
    ...
    @property
    def is_point(self):
        return self.layer_geometry in (GeometryTypes.Point, GeometryTypes.MultiPoint)

    @property
    def is_linestring(self):
        return self.layer_geometry in (
...
class Feature(BaseUpdatableModel):
    ...
    identifier = models.CharField(
        max_length=255, blank=False, null=False, default=uuid.uuid4
    )
    properties = JSONField(default=dict, blank=True)
    layer = models.ForeignKey(Layer, on_delete=models.PROTECT, related_name="features")

    source = models.IntegerField(
        null=True, blank=True, help_text="Internal field used by pgRouting"
    )
    ...
    def clean_vect_tile_cache(self):
        ...
        vtile.clean_tiles(
            ...
            self.layer.layer_settings_with_default("tiles", "pixel_buffer"),
            self.layer.layer_settings_with_default("tiles", "features_filter"),
            self.layer.layer_settings_with_default("tiles", "properties_filter"),
            self.layer.layer_settings_with_default("tiles", "features_limit"),
        )
...
class LayerRelation(models.Model):
    origin = models.ForeignKey(
        Layer, on_delete=models.PROTECT, related_name="relations_as_origin"
    )
    destination = models.ForeignKey(
        Layer, on_delete=models.PROTECT, related_name="relations_as_destination"
    )
...
class FeatureRelation(models.Model):
    origin = models.ForeignKey(
        Feature, on_delete=models.PROTECT, related_name="relations_as_origin"
    )
    destination = models.ForeignKey(
        Feature, on_delete=models.PROTECT, related_name="relations_as_destination"
    )
    properties = JSONField(default=dict, blank=True)
