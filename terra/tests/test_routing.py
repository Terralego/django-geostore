...
class RoutingTestCase(TestCase):
    points = [
        {"type": "Point", "coordinates": [1.4534568786621094, 43.622127847162005]},
        {"type": "Point", "coordinates": [1.4556884765625, 43.61839973326468]},
        {"type": "Point", "coordinates": [1.4647650718688965, 43.61916090863259]},
    ]
    ...
    def setUp(self):
        ...
        with open(geojson_path, mode="r", encoding="utf-8") as geojson:
    ...
    def test_points_in_line(self):
        routing = Routing(
            [
                Point(*p["coordinates"], srid=app_settings.INTERNAL_GEOMETRY_SRID)
    ...
    def test_routing_view_bad_geometry(self):
        response = self.client.post(reverse("terra:layer-route", args=[self.layer.pk]))
    ...
    def test_routing_view(self):
        points = [
            Point(*point["coordinates"], srid=app_settings.INTERNAL_GEOMETRY_SRID)
    ...
    def test_routing_view_edge_case(self):
        points = [
            Point(*p["coordinates"], srid=app_settings.INTERNAL_GEOMETRY_SRID)
            for p in [self.points[0], self.points[0]]
        ]
        ...
        # Ensure End Points are close to requested points
        start = Point(
            *response.get("geom").get("features")[0].get("geometry").get("coordinates")
        )
        end = Point(
            *response.get("geom").get("features")[-1].get("geometry").get("coordinates")
        )
        self.assertTrue(points[0].distance(start) <= 0.001)
        self.assertTrue(points[-1].distance(end) <= 0.001)
