...
class ImportGeojsonTest(TestCase):
    ...
    def test_bad_query(self, mocked_get):
        ...
        query = "bad query"
        self.assertRaises(
            CommandError, call_command, "import_osm", f"{query}", f"-t{type_feature}"
        )
    ...
    def test_good_query(self, mocked_get):
        ...
        output = StringIO()
        call_command(
            "import_osm", f"{query}", f"-t{type_feature}", "-v 1", stderr=output
        )
    ...
    def test_ogr2ogr_fail(self, mock_ogr2ogr_stdout, mocked_get):
        ...
        with self.assertRaises(CommandError) as error:
            call_command("import_osm", f"{query}", f"-t{type_feature}", "-v 0")
