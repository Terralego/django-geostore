...
class UpdateTopologyTestCase(TestCase):
    def test_update_topology_routing_fail(self):
        ...
        with self.assertRaises(CommandError) as error:
            call_command("update_topology", f"--layer-pk={layer.pk}", verbosity=0)
        self.assertEqual(
            "An error occuring during topology update", str(error.exception)
        )
    ...
    def test_update_topology_fail(self):
        ...
        with self.assertRaises(CommandError) as error:
            call_command(
                "update_topology", f"--layer-pk=999", verbosity=0, stdout=output
            )
    ...
    def test_update_topology(self):
        ...
        output = StringIO()
        call_command(
            "update_topology", f"--layer-pk={in_layer.pk}", verbosity=1, stdout=output
        )
