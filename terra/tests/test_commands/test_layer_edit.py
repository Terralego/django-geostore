...
class LayerEditTest(TestCase):
    def setUp(self):
        ...
        call_command(
            "import_shapefile", sample_shapefile.name, "-i", "ID_PG", verbosity=0
        )
    ...
    def test_layer_edit_guess_zoom(self):
        ...
        # Change settings
        call_command(
            "layer_edit", "-pk", layer.pk, "-l", "new_name", "-gr", "new_group", "-gz"
        )
        self.assertEqual(
            str(layer.settings), "{'tiles': {'maxzoom': 15, 'minzoom': 15}}"
        )
