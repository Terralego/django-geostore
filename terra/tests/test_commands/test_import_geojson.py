...
class ImportGeojsonTest(TestCase):
    def test_default_group(self):
        output = StringIO()
        call_command(
            "import_geojson", get_files_tests("empty.json"), verbosity=1, stdout=output
        )
    ...
    def test_schema_generated(self):
        call_command(
            "import_geojson", get_files_tests("bati.geojson"), "-gs", verbosity=0
        )
    ...
    def test_import_geojson_layer_with_bad_settings(self):
        ...
        with self.assertRaises(CommandError) as error:
            call_command("import_geojson", empty_geojson, "-ls", bad_json, verbosity=0)
        self.assertEqual(
            "Please provide a valid layer settings file", str(error.exception)
        )
    ...
    def test_import_geojson_layer_with_pk_layer(self):
        ...
        call_command(
            "import_geojson", f"--layer-pk={layer.pk}", geojson_sample, verbosity=0
        )
    ...
    def test_import_geojson_layer_with_wrong_pk_layer(self):
        ...
        with self.assertRaises(CommandError) as error:
            call_command(
                "import_geojson", f"--layer-pk=999", "-gs", geojson_sample, verbosity=0
            )
