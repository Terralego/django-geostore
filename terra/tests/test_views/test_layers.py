...
from terra.tests.factories import FeatureFactory, LayerFactory, UserFactory
...
class LayerLineIntersectionTestCase(TestCase):
    ...
    def test_intersect_bad_geometry(self):
        linestring = {"type": "LineString", "coordinates": [[[1.25, 5.5, [2.65, 5.5]]]]}
...
class LayerPolygonIntersectTestCase(TestCase):
    ...
    def test_polygon_intersection(self):
        ...
        Feature.objects.create(
            geom=json.dumps(
                {"type": "LineString", "coordinates": [[103.0, 0.0], [101.0, 1.0]]}
            ),
            layer=layer,
            properties={},
        )
        Feature.objects.create(
            geom=json.dumps({"type": "Point", "coordinates": [100.0, 0.0]}),
            layer=layer,
            properties={},
        )
        ...
        self.assertEqual(2, len(json_response["results"]["features"]))


class LayerFeatureIntersectionTest(TestCase):
    def setUp(self):
        self.fake_geometry = {"type": "Point", "coordinates": [2.0, 45.0]}
    ...
    def test_features_intersections(self):
        layer = LayerFactory()
        FeatureFactory(
            layer=layer, geom=GEOSGeometry(json.dumps(self.intersect_ref_geometry))
        )
        ...
        self.assertDictEqual(
            self.intersect_ref_geometry, response.get("features")[0].get("geometry")
        )
...
class LayerShapefileTestCase(TestCase):
    ...
    def test_properties_serializations(self):
        layer = LayerFactory()
        test_properties = {"int": 42, "str": "test string", "dict": {"a": "b"}}
    ...
    def test_shapefile_same_import_export(self):
        ...
        FeatureFactory(layer=self.layer, properties={"key1": [{"key3": "hello world"}]})
        ...
        self.assertEqual(HTTP_200_OK, response.status_code)
        self.assertEqual(
            self.layer.features.first().properties,
    ...
    def test_shapefile_no_permission(self):
        ...
        self.assertEqual(self.client.get(shape_url).status_code, HTTP_403_FORBIDDEN)
    ...
    def test_no_shapefile_import(self):
        ...
        response = self.client.post(reverse("terra:layer-shapefile", args=[layer.pk]))
    ...
    def test_shapefile_import_view_error(self):
        ...
        shapefile = SimpleUploadedFile("shapefile-WGS84.zip", b"bad bad data")
...
class LayerDetailTest(TestCase):
    ...
    def test_update(self):
        ...
        feature = FeatureFactory(layer=self.layer, geom=geom, properties={"a": "b"})

        updated_properties = {"c": "d", "a": "d"}
        ...
        self.assertEqual(HTTP_200_OK, response.status_code)
        response = response.json()
        self.assertEqual(response["features"][0]["properties"], updated_properties)
...
class LayerCreationTest(TestCase):
    ...
    def test_point_layer_allow_point(self):
        response = self.client.post(
            reverse("terra:feature-list", args=[self.point_layer.pk]),
