...
class Command(LayerCommandMixin, BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        exclusive_group_processing.add_argument(
            ...
            action="store",
            help=(
                "UNSAFE SQL Query. Eg. SELECT identifier, properties, ST_Centroid(geom::geometry) AS geom FROM in0"
            ),
        )
        exclusive_group_processing.add_argument(
            "-s-centroid",
        ...
        exclusive_group_processing.add_argument(
            ...
            "--python",
            help=(
                "UNSAFE Compute with a python callable module, use multiple key=value as arguments"
            ),
        )
        exclusive_group_processing.add_argument(
            "-mv",
    ...
    def _get_layer_ins(self, pks, names):
        try:
            ...
            raise CommandError(
                f"Fails open one or many layers layer-pk-ins: {', '.join(pks)} "
    ...
    def handle(self, *args, **options):
        ...
        layer_ins = self._get_layer_ins(
            options.get("layer_pk_ins"), options.get("layer_name_ins")
        )
        ...
        command_args = dict(
            [a.split("=", 1) for a in options.get("command_args") or []]
        )
